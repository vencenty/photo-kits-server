...
func (m *defaultPhotoModel) Insert(ctx context.Context, data *Photo) (sql.Result, error) {
	...
	ret, err := m.conn.ExecCtx(ctx, query, data.OrderId, data.Url, data.OriginUrl, data.ThumbUrl, data.Spec, data.IsResized, data.Error, data.Status)
	return ret, err
}
...
func (m *defaultPhotoModel) Update(ctx context.Context, data *Photo) error {
	...
	_, err := m.conn.ExecCtx(ctx, query, data.OrderId, data.Url, data.OriginUrl, data.ThumbUrl, data.Spec, data.IsResized, data.Error, data.Status, data.Id)
	return err
}
