...
type (
	...
	Photo struct {
		...
		Url       string    `db:"url"`
		OriginUrl string    `db:"origin_url"` // 源地址
		ThumbUrl  string    `db:"thumb_url"`  // 缩略图url
		Num       int64     `db:"num"`        // 数量。洗几张，默认是1
...
func (m *defaultPhotoModel) Insert(ctx context.Context, data *Photo) (sql.Result, error) {
	...
	ret, err := m.conn.ExecCtx(ctx, query, data.OrderId, data.Url, data.OriginUrl, data.ThumbUrl, data.Num, data.Spec, data.IsResized, data.Error, data.Status)
	return ret, err
}
...
func (m *defaultPhotoModel) Update(ctx context.Context, data *Photo) error {
	...
	_, err := m.conn.ExecCtx(ctx, query, data.OrderId, data.Url, data.OriginUrl, data.ThumbUrl, data.Num, data.Spec, data.IsResized, data.Error, data.Status, data.Id)
	return err
}
