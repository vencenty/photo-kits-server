...
type (
	...
	Order struct {
		...
		LastError  string    `db:"last_error"`  // 最后一次错误信息
	}
)
...
func (m *defaultOrderModel) Insert(ctx context.Context, data *Order) (sql.Result, error) {
	...
	ret, err := m.conn.ExecCtx(ctx, query, data.OrderSn, data.Receiver, data.Remark, data.Status, data.RetryCount, data.LastError)
	return ret, err
}

func (m *defaultOrderModel) Update(ctx context.Context, newData *Order) error {
	query := fmt.Sprintf("update %s set %s where `id` = ?", m.table, orderRowsWithPlaceHolder)
	_, err := m.conn.ExecCtx(ctx, query, newData.OrderSn, newData.Receiver, newData.Remark, newData.Status, newData.RetryCount, newData.LastError, newData.Id)
	return err
}
